...
def cea():
    ...
    if 1:
        @np.vectorize
        def cea_lut_Ivac(P, ofr, eps):
            P *= 1e6
            table = [ [ [ 126.1773681640625,  174.53619384765625 ],
            ...
            ofr = (ofr - 0.5) / (13 - 0.5) * (len(table) - 1);
            i = int(ofr)
            if (i < 0):
                i = 0
            if (i > len(table) - 2):
            ...
            jj = -1.4 / (P * 1e-6 + 1.1081069) + 1.1685101;
            kk = -3.3 / (eps + 1.4498447) + 1.245356;
            ...
            return (v000 * (1.0 - ii) * (1.0 - jj) * (1.0 - kk)
                ...
                + v110 *        ii  *        jj  * (1.0 - kk)
                + v111 *        ii  *        jj  *        kk);
        def error_lut_Pofr(P, ofr, eps=4.0):
            P = P.ravel()
            ofr = ofr.ravel()
            approx = cea_lut_Ivac(P, ofr, eps)
            real = CEA["ivac"](P, ofr, eps)
            return 100.0 * np.abs(approx / real - 1)

        if 0:
            peek3d(
                [map_Pofr(CEA["ivac"], eps=4.0), *CEA.in_Pofr(N=50)],
                [map_Pofr(cea_lut_Ivac, eps=4.0), *CEA.in_Pofr(N=50)],
                [error_lut_Pofr, *CEA.in_Pofr(N=50)],
            )
            return
...
(0, 1, 2, 4, 5) / (1, 2, 4, 5, 8) ..................... 4.685%
cea_y_throat_high -> (-3.0848 + 11.081 x - 6.1412 y - 0.83732 xy + y^2) / (7.7625 x - 5.8353 y - 0.36104 xy + 0.89251 y^2 - 0.018525 xy^2)
    f64 x1 = ;
    f64 y1 = ;
    f64 x1y1 = ;
    f64 y2 = ;
...
(0, 1, 2, 3, 4, 5) / (3, 4, 5, 6, 7, 12) .............. 6.098%
cea_y_throat_low -> (-0.11058 + 0.099516 x + 0.026734 y + 0.5838 x^2 - 1.401 xy + y^2) / (0.4818 x^2 - 1.0447 xy + 0.76162 y^2 + 0.00037829 x^3 - 0.017671 x^2 y + 0.0017254 x^2 y^2)
    f64 x1 = ;
    f64 y1 = ;
    f64 x2 = ;
    f64 x1y1 = ;
